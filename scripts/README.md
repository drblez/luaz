...
Формирует план скачивания спулов из JES (список JESID и имён локальных файлов).  
Условия: Python 3, входной список от `dir` в FTP JES.  
Использование: вызывается из `ftp_submit.sh`.
...
Опции: `--full` форсирует полный аплоад независимо от state‑файла.  
Связанные документы: `docs/DEV_WORKFLOW.md`.
...
исключает `*.md` из синхронизации,  
режим `--file` для быстрой синхронизации одного файла, инкрементальная  
синхронизация по хэшам (state‑файл `MF_SYNC_STATE` или  
`$XDG_CACHE_HOME/luaz/ftp_sync_state.json`).  
Опции: `--full` игнорирует state‑файл и загружает все файлы.  
Связанные документы: `docs/DEV_WORKFLOW.md`.
